#!/usr/bin/env python3
"""
Prepare MathArena/hmmt_feb_2025 (HMMT February 2025) for OPSD evaluation (or training).

HMMT February 2025 dataset has 30 competition problems.
Fields: problem_idx, problem, answer, problem_type
...
Usage:
  ...
  python prepare_hmmt2025.py --mode eval --num-samples 30 \\
      --output /root/data/hmmt2025_eval.jsonl --seed 42
...
HMMT2025_REPO = "MathArena/hmmt_feb_2025"
...
def _row_to_entry(row: dict, pcol: str, acol: str) -> dict | None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare HMMT February 2025 for OPSD evaluation or training.",
    ...
    parser.add_argument("--num-samples", type=int, required=True,
                        help="Number of samples to output (HMMT 2025 has 30 total)")
    ...
    print(f"Loading '{HMMT2025_REPO}' ...")
    ds = hf.load_dataset(HMMT2025_REPO, cache_dir=cache_dir, trust_remote_code=True)
    ...
    with out_path.open("w", encoding="utf-8") as fout:
        for row in all_rows:
            if written >= args.num_samples:
                break
            entry = _row_to_entry(row, pcol, acol)
            if entry is None:
                skipped_malformed += 1
                continue
