...
# Token-KL-Weighted OPSD (no segmentation, confidence-weighted JSD only)
# Usage: bash examples/on_policy_distillation/run-qwen3-8B-token-kl-weighted.sh
#
# Compared to run-qwen3-8B-segmented-opsd.sh:
#   - SEG_CHUNK_SIZE > rollout_max_response_len  →  always 1 segment per response
#     →  segment-position weighting is skipped (len(segments)==1 guard in code)
#   - KL_WEIGHT_MODE=linear  →  per-token confidence weighting still active
#     (high-entropy teacher tokens get lower weight, low-entropy get higher weight)
#
# This isolates the contribution of token-level confidence weighting alone.
...
CKPT_SAVE_DIR="/root/output/Qwen3-8B_token_kl_weighted_linear"
CKPT_ARGS=(
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-8B-token-kl-weighted-linear
